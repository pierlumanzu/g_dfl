    ...
    with session.as_default():
        ...
        for alg in args.algs:
            ...
            if alg == 'G-DFL':
                ...
                for seed in args.seeds:
                    ...
                    for tgd in args.type_gradient_related_direction:
                        ...
                        for mci in args.max_continuous_iter:
                            ...
                            print("alg: {} | prob: {} | seed: {} | tgd: {} | mci: {}".format(alg, problem.problem_name(), seed, tgd, mci))

                            start_time = time.time()
                            x, _, stop_status, nf, best_nf, n_iter, best_it, best_time, ng, best_ng = G_DFL(args.verbose, 
                                                                                                            ...
                                                                                                            args.eta_for_discrete_dir, args.xi_for_discrete_dir, args.min_xi_for_discrete_dir).run(seed, tgd, mci, problem, start_time)
                ...
                if args.save_logs:
                    sys.stdout = open(os.path.join("Outputs", "{}.txt".format(alg)), "a", buffering=1)

                print("alg: {} | prob: {}".format(alg, problem.problem_name()))

                start_time = time.time()
                x, _, stop_status, nf, best_nf, n_iter, best_it, best_time, ng, best_ng = MISQP(args.verbose, 
                                                                                                args.max_time * 60 if args.max_time is not None else np.inf,
                                                                                                args.tolerance_for_best,
                                                                                                args.initial_delta_c, args.initial_delta_i, 
                                                                                                args.epsilon, args.M).solve(problem, start_time)
                elapsed_time = time.time() - start_time
